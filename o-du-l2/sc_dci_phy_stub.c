...
int main(){
	...
	while(1){
		...
		printf("INFO   -->  PHY_STUB : channel_output\n");
                    Py_Initialize();
         			PyObject *noise = Py_BuildValue("s", "/home/eric/semantic/SemanticRL/Evaluation/channel.py");
         			FILE *channel = _Py_fopen_obj(noise, "r+");
         			if(channel != NULL) {
            		PyRun_SimpleFile(channel, "/home/eric/semantic/SemanticRL/Evaluation/channel.py");
         			}
		printf("INFO   -->  PHY_STUB : decoder_output\n");
                    Py_Initialize();
                    PyObject *decode = Py_BuildValue("s", "/home/eric/semantic/SemanticRL/Evaluation/decoder.py");
                    FILE *decoder = _Py_fopen_obj(decode, "r+");
                    if(decoder != NULL) {
                    PyRun_SimpleFile(decoder, "/home/eric/semantic/SemanticRL/Evaluation/decoder.py");
                    }
		printf("INFO   -->  PHY_STUB : semanticRL_example1_candidate_sentence\n");
                    Py_Initialize();
                    PyObject *sentence = Py_BuildValue("s", "/home/eric/semantic/SemanticRL/Evaluation/candidate_sentence.py");
                    FILE *candidate = _Py_fopen_obj(sentence, "r+");
                    if(candidate != NULL) {
                    PyRun_SimpleFile(candidate, "/home/eric/semantic/SemanticRL/Evaluation/candidate_sentence.py\n");
                    }
